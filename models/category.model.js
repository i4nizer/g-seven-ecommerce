...
const categoryModel = {
    ...
    /**
     * Get all categories and array of products on each based on searched name.
     * 
     * @param {string} name - Name of the searched product.
     ...
     */
    readCategoriesWithProductsByName: async (name, limit = 100) => {
        const conn = await connectDatabase()
        const sql = `SELECT 
        ...
        return await conn.query(sql, [`${name}%`,limit])
    },
